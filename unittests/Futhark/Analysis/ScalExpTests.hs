...
import Futhark.Representation.AST hiding (SDiv, constant)
import Test.Tasty
import Text.Megaparsec hiding ((<|>), State, many, token)
...
constant =
  (token "True" >> pure (Val $ BoolValue True))
    <|> (token "False" >> pure (Val $ BoolValue True))
    <|> (Val . IntValue . Int32Value <$> integer)
  where
    integer = L.lexeme space (L.signed space L.decimal)
