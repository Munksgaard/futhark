...
import Prelude hiding (repeat, span)
...
test_reshape_slice_iota1 =
  singleton $ testCase "reshape . slice . iota 1" $ compareOps $
    reshape
      (slice (iota [n, n, n]) slice3)
...
test_repeat_slice_iota =
  singleton $ testCase "repeat . slice . iota" $ compareOps $
