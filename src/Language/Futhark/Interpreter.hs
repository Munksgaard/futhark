...
import Futhark.Representation.Primitive (floatValue, intValue)
...
newtype EvalM a
  = EvalM
      ( ReaderT (Stack, M.Map FilePath Env)
  ...
  if ok
    then
      returned env t retext
        $ toArray' ShapeLeaf
        $ map toInt [start', start' + step .. end_adj]
...
initialCtx =
  ...
  where
    ...
    def "map" = Just $ TermPoly Nothing $ \t -> return $ ValueFun $ \v ->
      ...
      where
        typeRowShape = traverse id . structTypeShape mempty . stripArray 1
