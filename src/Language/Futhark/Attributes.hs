...
traverseDims f = go PosImmediate
  where
    go :: forall als'. DimPos -> TypeBase fdim als' -> f (TypeBase tdim als')
...
leadingOperator s =
  maybe Backtick snd $ find ((`isPrefixOf` s') . fst)
