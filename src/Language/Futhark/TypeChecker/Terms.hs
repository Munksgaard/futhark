        ...
        case f of
          ...
          Just fname ->
            "Cannot apply" <+> pquote (ppr fname) <+> "to"
              <+> pquote (shorten $ pretty $ flatten $ ppr e)
              <> " (invalid type)."
...
checkUnmatched e = void $ checkUnmatched' e >> astMap tv e
  where
    ...
    tv =
      ASTMapper
        { mapOnExp = \e' -> checkUnmatched' e' >> return e',
          mapOnName = pure,
          mapOnQualName = pure,
...
checkApply loc fname tfun@(Scalar TypeVar {}) arg = do
  ...
  unify (mkUsage loc "use as function") (toStruct tfun)
    $ Scalar
    $ Arrow mempty Unnamed (toStruct (argType arg)) tv
  ...
  typeError loc mempty $
    if prev_applied == 0
      then
        ...
        "Cannot apply" <+> fname' <+> "to argument #"
          <> ppr (prev_applied + 1)
...
dimUses = execWriter . traverseDims f
  where
    f PosImmediate (NamedDim v) = tell (S.singleton (qualLeaf v), mempty, mempty)
