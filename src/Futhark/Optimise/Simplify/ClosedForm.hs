...
foldClosedForm look pat lam accs arrs = do
  ...
  letBindNames_ [isEmpty]
    $ BasicOp
    $ CmpOp (CmpEq int32) inputsize (intConst Int32 0)
  letBind_ pat
    =<< ( If (Var isEmpty)
            <$> resultBodyM accs
...
loopClosedForm pat merge i bound body = do
  ...
  letBindNames_ [isEmpty]
    $ BasicOp
    $ CmpOp (CmpSlt Int32) bound (intConst Int32 0)
  letBind_ pat
    =<< ( If (Var isEmpty)
  ...
  where
    ...
    properIntSize t =
      Just $ letSubExp "converted_size"
        $ BasicOp
        $ ConvOp (SExt Int32 t) size
    properFloatSize t =
      Just $ letSubExp "converted_size"
        $ BasicOp
        $ ConvOp (SIToFP Int32 t) size
...
determineKnownBindings look lam accs arrs =
  ...
  where
    ...
    arrBnds =
      M.fromList $ mapMaybe isReplicate $
