...
bindableSimpleOps = SimpleOps mkExpAttrS' mkBodyS' mkLetNamesS' protectHoistedOpS'
  where
    mkExpAttrS' _ pat e = return $ mkExpAttr pat e
...
newtype SimpleM lore a
  = SimpleM
      ( ReaderT (SimpleOps lore, Env lore)
...
bindLoopVar var it bound =
  ...
  where
    clampVar = ST.insertLoopVar var it bound
    -- If we enter the loop, then 'bound' is at least one.
    clampUpper = case bound of
      Var v -> ST.isAtLeast v 1
      _ -> id
...
protectIf
  ...
  ( Let
      pat
      (StmAux cs _)
      (If cond taken_body untaken_body (IfAttr if_ts IfFallback))
    ) = do
...
constructBody stms res =
  fmap fst $ runBinder $ insertStmsM $ do
    addStms stms
    resultBodyM res
...
hoistCommon cond ifsort ((res1, usages1), stms1) ((res2, usages2), stms2) = do
  ...
  let -- We are unwilling to hoist things that are unsafe or costly,
      ...
      hoistbl_nms =
        filterBnds desirableToHoist getArrSz_fun
          $ stmsToList
          $ stms1 <> stms2
      ...
      block =
        branch_blocker
          ...
          `orIf` isInPlaceBound
          `orIf` isNotHoistableBnd hoistbl_nms
  rules <- asksEngineEnv envRules
  (body1_bnds', safe1) <-
  ...
  where
    filterBnds interesting getArrSz_fn all_bnds =
