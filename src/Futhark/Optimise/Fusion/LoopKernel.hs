  ...
  let mapLikeFusionCheck =
        let (res_lam, new_inp) = fuseMaps unfus_set lam_p inp_p_arr outPairs lam_c inp_c_arr
            (extra_nms, extra_rtps) =
              unzip $ filter ((`nameIn` unfus_set) . fst)
                $ zip outVars
                $ map (stripArray 1)
                $ SOAC.typeOf soac_p
      ...
      SOAC.Screma _ (ScremaForm (scan_lam_p, scan_nes_p) reds_p _) _
        ...
        | mapFusionOK (drop (length scan_nes_p + Futhark.redResults reds_p) outVars) ker
          ...
          success
            ...
            $ SOAC.Screma
              w
              (ScremaForm (scan_lam', scan_nes_p ++ scan_nes_c) (reds_p ++ reds_c) res_lam')
              new_inp
    ------------------
    -- Scatter fusion --
    ------------------
      ...
      SOAC.Hist _ ops_p _ _
        ...
        | horizFuse -> do
          let p_num_buckets = length ops_p
              ...
              body' =
                Body
                  { bodyAttr = bodyAttr body_p, -- body_p and body_c have the same lores
    ...
    ( SOAC.Scatter _len2 _lam_c ivs2 as2,
      SOAC.Scatter _len_p _lam_p ivs_p as_p
        ...
        | horizFuse -> do
          ...
          let body' =
                Body
                  { bodyAttr = bodyAttr body_p, -- body_p and body2 have the same lores
    ...
    let map_body =
          mkBody
            ( oneStm
                ...
                $ Op
                $ Futhark.Screma w (ScremaForm (scan_fun', nes') [] id_map_lam) arrs'
            )
            $ map Var
            $ patternNames map_pat
        map_fun' = Lambda map_params map_body map_rettype
        perm = case lambdaReturnType map_fun of
          [] -> []
          t : _ -> 1 : 0 : [2 .. arrayRank t]
    return
      ( SOAC.Screma map_w (ScremaForm (nilFn, mempty) [] map_fun') map_arrs',
