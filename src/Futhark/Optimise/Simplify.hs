...
simplifyLambda ::
  (MonadFreshNames m, HasScope lore m, Engine.SimplifiableLore lore) =>
  Engine.SimpleOps lore ->
  RuleBook (Engine.Wise lore) ->
  Engine.HoistBlockers lore ->
  Lambda lore ->
...
simplifyLambda simpl rules blockers orig_lam args =
  ...
  where
    f lam' = Engine.simplifyLambdaNoHoisting lam' args
...
simplifyStms = simplifySomething f g
  where
    f stms = fmap snd $ Engine.simplifyStms stms $ return ((), mempty)
