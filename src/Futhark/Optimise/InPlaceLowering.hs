...
optimiseBody (Body als bnds res) = do
  bnds' <-
    deepen $ optimiseStms (stmsToList bnds) $
      mapM_ seen res
...
optimiseStms (bnd : bnds) m = do
  ...
  where
    ...
    checkIfForwardableUpdate
      bnd'@( Let
               (Pattern [] [PatElem v attr])
               (StmAux cs _)
               e
             )
      bnds'
  ...
  bindingScope (scopeOfSegSpace (segSpace op)) $ do
    let mapper = identitySegOpMapper {mapOnSegOpBody = onKernelBody}
        onKernelBody kbody = do
          stms <-
            deepen $ optimiseStms (stmsToList (kernelBodyStms kbody))
              $ mapM_ seenVar
              $ namesToList
              $ freeIn
...
maybeForward v dest_nm dest_attr cs src i = do
  ...
  if available && certs_available && samebody && optimisable && not_prim
    then do
