...
fuseFun fun = do
  stms <-
    fuseStms
      (scopeOfFParams (funDefParams fun))
...
greedyFuse ::
  ...
  Names ->
  FusedRes ->
  (Pattern, Certificates, SOAC, Names) ->
  FusionGM FusedRes
greedyFuse rem_bnds lam_used_nms res (out_idds, cs, orig_soac, consumed) = do
  ...
  if not fusible_ker
    then addNewKerWithInfusible res (patternIdents out_idds, cs, soac, consumed) ufs
    else do
...
prodconsGreedyFuse res (out_idds, cs, soac, consumed) = do
  ...
  where
    certifyKer k = k {certificates = certificates k <> cs}
      ...
      lam_body <- runBodyBinder $ localScope (scopeOfLParams lam_params) $ do
        ...
        eBody
          [ pure $
            ...
            pure
              $ BasicOp
              $ BinOp (Add Int32) (Futhark.Var offset) (Futhark.Var chunk_size)
          ]
      let lam =
            Lambda
  ...
  case soac of
    Futhark.Screma w (Futhark.ScremaForm (scan_lam, scan_nes) reds map_lam) arrs -> do
      ...
      return $
        Futhark.Screma
          w
          (Futhark.ScremaForm (Aliases.removeLambdaAliases scan_lam, scan_nes) reds' map_lam')
          arrs'
