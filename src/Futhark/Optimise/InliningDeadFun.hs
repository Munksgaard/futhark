...
aggInlineFunctions cg =
  ...
  where
    ...
    callsRecursive fd =
      maybe False (any recursive . M.keys) $
...
inlineFunction always_reshape pat aux args (_, safety, loc, locs) fun =
  ...
  where
    ...
    param_stms =
      zipWith
        (reshapeIfNecessary param_names)
    ...
    res_stms =
      certify (stmAuxCerts aux)
        <$> zipWith
          (reshapeIfNecessary (patternNames pat))
          (patternIdents pat)
...
addLocations caller_safety more_locs = fmap onStm
  where
    onStm stm = stm {stmExp = onExp $ stmExp stm}
...
aggInlineConstants orig_fds =
  ...
  where
    fdmap = M.fromList $ zip (map funDefName orig_fds) orig_fds
    inlineInEntry fd =
    ...
    constsInStms prev stms =
      case stmsHead stms of
        Nothing -> mempty
        Just
          ( Let pat aux (Apply fname args _ prop),
            stms'
            )
...
inlineConstants =
  Pass
    { passName = "Inline constants",
      passDescription = "Inline and remove dead constants.",
      passFunction = pass
    }
  where
    pass prog = return $ Prog $ aggInlineConstants $ progFuns prog
