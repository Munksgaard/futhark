              ...
              return $
                update
                  ...
                  ( mempty,
                    row_copy,
                    src2attr
                      `setType` stripArray (length is2) (typeOf src2attr),
                    []
                  )
                  substs'
...
substituteIndicesInVar substs v
  ...
  | Just (cs2, src2, src2_attr, is2) <- lookup v substs =
    certifying cs2
      ...
      $ BasicOp
      $ Index src2
      $ fullSlice (typeOf src2_attr) is2
  | otherwise =
    return v
...
substituteIndicesInBody substs body = do
  (substs', bnds') <-
    inScopeOf bnds
      $ collectStms
      $ substituteIndicesInStms substs bnds
  (ses, ses_bnds) <-
    inScopeOf bnds'
      $ collectStms
      $ mapM (substituteIndicesInSubExp substs')
      $ bodyResult body
