...
lowerUpdateIntoKernel update kspace kbody = do
  ...
  where
    DesiredUpdate _bindee_nm bindee_attr _cs src is _val = update
        ...
        tell
          ( [ mkLet [] [Ident source source_t] $ BasicOp
            ...
            [ mkLet [] [elmident] $ BasicOp $
                Index
                  (updateName update)
                  (fullSlice (typeOf $ updateType update) $ updateIndices update)
            ]
          )
        return $
          Right
