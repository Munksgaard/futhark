  ...
  where
    invariantTo names stm =
      case patternNames (stmPattern stm) of
        ...
        v : _ ->
          not $ any (`nameIn` names) $ namesToList $
...
injectPrelude initial_space variance prestms used (host_stms, tiling, tiledBody) =
  ...
  where
    ...
    tiledBody' privstms = do
      ...
      let live_set =
            namesToList $ liveSet precomputed_variant_prestms $
      ...
      tiledBody' privstms = inScopeOf host_stms $ do
        ...
        let indexMergeParams slice =
              localScope (scopeOfFParams mergeparams')
                $ forM_ (zip mergeparams mergeparams')
                $ \(to, from) ->
      ...
      then
        ...
        group_size <-
          letSubExp "computed_group_size"
            $ BasicOp
            $ BinOp (SMin Int32) (unCount (segGroupSize initial_lvl)) kdim
...
readTile2D (kdim_x, kdim_y) (gtid_x, gtid_y) (gid_x, gid_y) tile_size num_groups group_size kind privstms tile_id arrs_and_perms =
  segMap2D
    "full_tile"
    (SegThread num_groups group_size SegNoVirt)
    ResultNoSimplify
    (tile_size, tile_size)
    $ \(ltid_x, ltid_y) -> do
      i <-
    ...
    segMap2D
      ...
      $ \(ltid_x, ltid_y) -> do
        ...
        tiles' <- forM tiles_and_perms $ \(tile, perm) -> do
          ...
          letExp "tile" $ BasicOp $ Index tile $
            sliceAt
              tile_t
              (head perm)
