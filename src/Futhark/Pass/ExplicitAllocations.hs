...
import Data.List (partition, sort, zip4)
...
class
  (MonadFreshNames m, HasScope lore m, ExplicitMemorish lore) =>
...
instance
  Allocable fromlore ExplicitMemory =>
  Allocator ExplicitMemory (AllocM fromlore ExplicitMemory)
  where
  expHints e = do
    f <- asks envExpHints
    f e
...
runAllocM handleOp hints (AllocM m) =
  ...
  where
    env = AllocEnv mempty False DefaultSpace handleOp hints
...
arraySizeInBytesExp t =
  ...
  where
    toInt64 = ConvOpExp $ SExt Int32 Int64
...
allocsForStm sizeidents validents e = do
  ...
  return
    ( Let (Pattern ctxElems valElems) (defAux ()) e,
      postbnds
    )
...
summaryForBindage t (Hint ixfun space) = do
  ...
  bytes <-
    computeSize "bytes" $
      product
        [ ConvOpExp (SExt Int32 Int64) (product (IxFun.base ixfun)),
    ...
    allocInMergeParam (mergeparam, Var v)
      | Array bt shape u <- paramDeclType mergeparam = do
        ...
        if space /= Space "local"
          ...
          then
            return
              ( mergeparam {paramAttr = MemArray bt shape Unique $ ArrayIn mem ixfun},
                lift . ensureArrayIn (paramType mergeparam) mem ixfun
              )
          else do
            def_space <- asks allocSpace
...
ensureArrayIn t mem ixfun (Var v) = do
  ...
  if src_mem == mem && src_ixfun == ixfun
    then return $ Var v
      ...
      copy <- newIdent (baseString v ++ "_ensure_copy") t
      let summary =
            MemArray (elemType t) (arrayShape t) NoUniqueness $
              ArrayIn mem ixfun
...
memoryInRetType ts = evalState (mapM addAttr ts) $ startOfFreeIDRange ts
  where
    addAttr (Prim t) = return $ MemPrim t
...
allocInFun (FunDef entry fname rettype params fbody) =
  runAllocM handleHostOp kernelExpHints
    $ allocInFParams (zip params $ repeat DefaultSpace)
    $ \params' -> do
      fbody' <-
        insertStmsM $
          allocInFunBody
  ...
  where
    ...
    inspect (Array pt shape u) space =
      let space' = fromMaybe DefaultSpace space
          bodyret =
            MemArray pt shape u $ ReturnsNewBlock space' 0
              $ IxFun.iota
              $ map convert
              $ shapeDims shape
       in bodyret
    ...
    alloc x y =
      case paramType x of
        Array bt shape u -> do
          ...
          return
            ( x {paramAttr = MemArray bt shape u $ ArrayIn mem ixfun_x},
              y {paramAttr = MemArray bt shape u $ ArrayIn mem ixfun_y}
            )
        Prim bt ->
          return
            ( x {paramAttr = MemPrim bt},
              y {paramAttr = MemPrim bt}
            )
        Mem space ->
          return
            ( x {paramAttr = MemMem space},
...
simplifiable simplifyInnerOp =
  ...
  where
    ...
    mkLetNamesS' vtable names e = do
      (pat', stms) <-
        runBinder $ bindPatternWithAllocations env names $
          removeExpWisdom e
      return (mkWiseLetStm pat' (defAux ()) e, stms)
      where
        env = removeScopeWisdom $ ST.toScope vtable
    protectOp taken pat (Alloc size space) = Just $ do
      ...
      size' <-
        letSubExp "hoisted_alloc_size"
          ...
          $ IfAttr [MemPrim int64] IfFallback
...
kernelExpHints (BasicOp (Manifest perm v)) = do
  ...
  let perm_inv = rearrangeInverse perm
      ...
      ixfun =
        IxFun.permute
          (IxFun.iota $ map (primExpFromSubExp int32) dims')
          perm_inv
...
mapResultHint lvl space = hint
  where
    num_threads =
      primExpFromSubExp int32 (unCount $ segNumGroups lvl)
  ...
  where
    maybePrivate t
      | Just (Array pt shape _) <- hasStaticShape t,
        ...
        let ixfun =
              IxFun.iota $ map (primExpFromSubExp int32) $
                shapeDims shape
