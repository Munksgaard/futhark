...
worthIntraGroup lam = interesting $ lambdaBody lam
  where
    interesting body =
      not (null $ nestedParallelism body)
...
worthSequentialising lam = interesting $ lambdaBody lam
  where
    interesting body = any (interesting' . stmExp) $ bodyStms body
