...
import Futhark.Pass.ExtractKernels.BlockedKernel (nonSegRed, segThread)
...
transformFunDef fundec = do
  ...
  where
    m = inScopeOf fundec $ transformBody $ funDefBody fundec
...
extractLambdaAllocations bound_outside lam = (lam {lambdaBody = body'}, allocs)
  where
    (body', allocs) = extractBodyAllocations bound_outside $ lambdaBody lam
...
extractStmAllocations bound_outside (Let (Pattern [] [patElem]) _ (Op (Alloc size space)))
  | expandable space,
    ...
    return Nothing
  where
    visibleOutside (Var v) = v `nameIn` bound_outside
...
expandedInvariantAllocations
  (num_threads64, Count num_groups, Count group_size)
  segspace
  invariant_allocs = do
      ...
      expand (mem, (per_thread_size, space)) = do
        ...
        return
          ( stmsFromList
              [ Let sizepat (defAux ())
                  $ BasicOp
                  $ BinOp (Mul Int64) num_threads64 per_thread_size,
                Let allocpat (defAux ())
                  $ Op
                  $ Alloc (Var total_size) space
              ],
...
unAllocScope = M.mapMaybe unInfo
  where
    unInfo (LetInfo attr) = LetInfo <$> unAttr attr
...
removeCommonSizes = M.toList . foldl' comb mempty . M.toList
  where
    comb m (mem, (size, space)) = M.insertWith (++) size [(mem, space)] m
  ...
  ((maxes_per_thread, size_sums), slice_stms) <- flip runBinderT kernels_scope $ do
    num_threads_64 <-
      letSubExp "num_threads"
        $ BasicOp
        $ ConvOp (SExt Int32 Int64) num_threads
    pat <-
      basicPattern []
        <$> replicateM
          num_sizes
    ...
    thread_space_iota <-
      letExp "thread_space_iota" $ BasicOp $
    ...
    size_sums <- forM (patternNames pat) $ \threads_max ->
      letExp "size_sum"
        $ BasicOp
        $ BinOp (Mul Int64) (Var threads_max) num_threads_64
