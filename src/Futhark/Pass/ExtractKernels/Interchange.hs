...
interchangeLoop
  ...
  (MapNesting pat cs w params_and_arrs) = do
    ...
    let lam = Lambda (params' <> new_params) body' rettype
        map_bnd =
          Let loop_pat_expanded (StmAux cs ())
            $ Op
            $ Screma w (mapSOAC lam)
            $ arrs' <> new_arrs
            ...
            | Just p <- find ((== v) . paramName) params = do
              ...
              return
                ( Var dummy,
                  oneStm
                    $ Let
                      (Pattern [] [PatElem dummy $ paramType p])
                      (defAux ())
                    $ BasicOp
                    $ SubExp
                    $ Var
  ...
  where
    isMapParameter v =
      fmap snd $ find ((== v) . paramName . fst)
        $ concatMap loopNestingParamsAndArrs
