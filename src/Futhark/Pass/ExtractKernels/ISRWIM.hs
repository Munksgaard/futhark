...
iswim res_pat w scan_fun scan_input
  | Just (map_pat, map_cs, map_w, map_fun) <- rwimPossible scan_fun = Just $ do
    ...
    let map_body =
          mkBody
            ( oneStm $ Let (setPatternOuterDimTo w map_pat) (defAux ())
                $ Op
                $ Screma w scan_soac scan_arrs
            )
            $ map Var
    ...
    addStm $ Let res_pat' (StmAux map_cs ()) $ Op $
      Screma
        map_w
        (ScremaForm (nilFn, mempty) [] map_fun')
        map_arrs'
    forM_
      ( zip
          (patternValueIdents res_pat)
    ...
    map_body <-
      case irwim red_pat w comm red_fun' red_input' of
        Nothing -> do
          ...
          return
            $ mkBody
              ( oneStm $ Let red_pat (defAux ())
                  $ Op
                  $ Screma w reduce_soac
                  $ map snd red_input'
              )
            $ map Var
            $ patternNames map_pat
