    ...
    Just ((pat, res), newtargets) -> do
      ...
      if null $ distStms acc'
        then return acc'
          ...
          let body = Body () (distStms acc') res
    ...
    repeatMissing arr inners = do
      ...
      if all (== Shape []) shapes
        then return (arr, mempty)
          ...
          return
            ( arr',
              oneStm $ Let (Pattern [] [PatElem arr' arr_t']) (defAux ())
                $ BasicOp
                $ Repeat outer_shapes inner_shape arr
            )
    ...
    Just (targets, distributed_bnds) ->
      tryDistributeStm nest targets bnd >>= \case
        ...
        Just (res, targets', new_kernel_nest) ->
          return $
            Just
              ( PostKernels [PostKernel distributed_bnds],
                res,
                new_kernel_nest,
                DistAcc
                  { distTargets = targets',
...
segmentedScatterKernel nest perm scatter_pat cs scatter_w lam ivs dests = do
  ...
  let nest' =
        pushInnerKernelNesting
          (scatter_pat, bodyResult $ lambdaBody lam)
          (MapNesting scatter_pat cs scatter_w $ zip (lambdaParams lam) ivs)
          nest
...
determineReduceOp ::
  (MonadBinder m, Lore m ~ Out.Kernels) =>
  Lambda ->
  [SubExp] ->
  m (Out.Lambda Out.Kernels, [SubExp], Shape)
...
isSegmentedOp nest perm segment_size free_in_op _free_in_fold_op nes arrs m = runMaybeT $ do
  ...
  where
    replicateMissing ispace inp = do
      ...
      letExp "repeated" $ BasicOp
        $ Repeat outer_shapes inner_shape
        $ kernelInputArray inp
