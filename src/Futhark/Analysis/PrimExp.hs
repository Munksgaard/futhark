...
instance Pretty v => Num (PrimExp v) where
  x + y
    | Just z <- msum [asIntOp Add x y, asFloatOp FAdd x y] = constFoldPrimExp z
    | otherwise = numBad "+" (x, y)

  x - y
    | Just z <- msum [asIntOp Sub x y, asFloatOp FSub x y] = constFoldPrimExp z
    | otherwise = numBad "-" (x, y)

  x * y
    | Just z <- msum [asIntOp Mul x y, asFloatOp FMul x y] = constFoldPrimExp z
    | otherwise = numBad "*" (x, y)
...
instance Pretty v => IntegralExp (PrimExp v) where
  x `div` y
    | Just z <- msum [asIntOp SDiv x y, asFloatOp FDiv x y] = constFoldPrimExp z
    | otherwise = numBad "div" (x, y)

  x `mod` y
    | Just z <- msum [asIntOp SMod x y] = z
  ...
  x `rem` y
    | Just z <- msum [asIntOp SRem x y] = constFoldPrimExp z
