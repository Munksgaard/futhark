...
import Data.List (genericReplicate, sort, sortBy)
...
data NNumExp
  = NSum [NNumExp] PrimType
  | NProd [ScalExp] PrimType
  deriving (Eq, Ord, Show)

data BTerm
  = NRelExp RelOp0 NNumExp
  | LogCt !Bool
  | PosId VName
  | NegId VName
  deriving (Eq, Ord, Show)
...
simplify e rangesrep = case runAlgSimplifier False (simplifyScal e) rangesrep of
  Left (Error err) ->
    error $
      "Error during algebraic simplification of: " ++ pretty e
        ++ "\n"
        ++ err
...
simplifyNRel inp_term@(NRelExp LTH0 inp_sofp) = do
  ...
  if in_gauss || isTrivialNRel term || tp `notElem` map IntType allIntTypes
    then return term
      ...
      ednf <- markGaussLTH0 $ gaussAllLTH0 True S.empty inp_sofp
      return $ case ednf of
...
gaussAllLTH0 static_only el_syms sofp = do
        ...
        Just (MaxMin ismin mmts) -> do
          ...
          if static_only
            ...
            --------------------------------------------------------------------
              ...
              if (fs_geq0 == Val (BoolValue True) && ismin)
                || (fs_leq0 == Val (BoolValue True) && not ismin)
                then-- at least one term should be < 0!
                do
                  let is_one_true = Val (BoolValue True) `elem` mms
                  ...
                  return $
                    if is_one_true
                      then Val (BoolValue True)
                        ...
                        if are_all_false
                          then Val (BoolValue False)
                          else RelExp LTH0 e_scal
                else-- otherwise all terms should be all true!
                do
                  let are_all_true = all (== Val (BoolValue True)) mms
            ...
            do
              let mm_fsgeq0 =
                    foldl
                      (if ismin then SLogOr else SLogAnd)
                      (Val (BoolValue (not ismin)))
                      mms
              let mm_fslth0 =
                    foldl
                      (if ismin then SLogAnd else SLogOr)
                      (Val (BoolValue ismin))
                      mms
  ...
  where
    ...
    findMinMaxTerm _ (NProd [MaxMin ismin e] _) =
      return (Just (MaxMin ismin e), [], [])
    findMinMaxTerm ii t@NProd {} = do
      (mm, fs) <- findMinMaxFact ii t
      return (mm, fs, [])
      ...
      case M.lookup ii rangesrep of
        Just (_, Just _, Just _) -> do
          ...
          case f of
            ...
            (Nothing, _) -> do
              (mm, fs', ts') <- findMinMaxTerm ii (NSum ts tp)
              return (mm, fs', t : ts')
        ...
        MaxMin ismin ts -> do
          ...
          if ii `nameIn` id_set
            then return (Just (MaxMin ismin ts), fs)
              ...
              (mm, fs') <- findMinMaxFact ii (NProd fs tp)
              return (mm, f : fs')
        _ -> do
          (mm, fs') <- findMinMaxFact ii (NProd fs tp)
          return (mm, f : fs')
      ...
      case M.lookup i rangesrep of
        ...
        Just (_, Just lb, Nothing) -> do
          ...
          case (and_half, aleq0) of
            ...
            _ -> do
              malmbm1lth0 <- gaussElimHalf static_only elsyms lb ma mbm1
        ...
        Just (_, Just lb, Just ub) ->
          if static_only
            then
              if aleq0 == Val (BoolValue True)
                then do
                  alpblth0 <- simplifyScal =<< gaussElimHalf static_only elsyms lb a b
                  if alpblth0 == Val (BoolValue True)
                    then return $ Just (Val (BoolValue True))
                    else do
                      maubmbm1 <- simplifyScal =<< gaussElimHalf static_only elsyms ub ma mbm1
                      return $
                        if maubmbm1 == Val (BoolValue True)
                          then Just (Val (BoolValue False))
                          else Nothing
                else
                  if ageq0 == Val (BoolValue True)
                    then do
                      aupblth0 <- simplifyScal =<< gaussElimHalf static_only elsyms ub a b
                      if aupblth0 == Val (BoolValue True)
                        then return $ Just (Val (BoolValue True))
                        else do
                          malbmbm1 <- simplifyScal =<< gaussElimHalf static_only elsyms lb ma mbm1
                          return $
                            if malbmbm1 == Val (BoolValue True)
                              then Just (Val (BoolValue False))
                              else Nothing
                    else return Nothing
            else do
...
pickSymToElim rangesrep elsyms0 e_scal =
  ...
  let ids0 = namesToList $ freeIn e_scal
      ...
      ids2 =
        filter
          ( \s -> case M.lookup s rangesrep of
              Nothing -> False
              Just _ -> True
          )
          ids1
...
simplifyScal (MaxMin ismin es) = do
  -- helperMinMax ismin  es pos
  -- pos <- asks pos
  ...
  let evals = filter isValue es0
      ...
      mvv = case evals of
        [] -> Nothing
        v : vs ->
          let myop = if ismin then min else max
              myval = getValue v
        ...
        case e2' of
          NProd fs tp -> do
            ...
            case e1Split of
              [] -> Val <$> getZero tp
              _ -> do
                (fs', e1Split') <- trySimplifyDivRec fs [] e1Split
                if length fs' == length fs
                  then turnBackAndDiv e1' e2' -- insuccess
                  else do
                    terms_e1' <- mapM joinTerm e1Split'
                    e1'' <- fromNumSofP $ NSum terms_e1' tp
                    case fs' of
                      [] -> return e1''
                      _ -> do
                        e2'' <- fromNumSofP $ NProd fs' tp
                        return $ SDiv e1'' e2''
        ...
        then return e1'
          ...
          (_, Val (IntValue n)) ->
            if P.intToInt64 n >= 1
              ...
              do
                new_e <- fromNumSofP $ NProd (genericReplicate (P.intToInt64 n) e1') tp
                simplifyScal new_e
...
negateBTerm (NRelExp rel e) = do
  ...
  case (tp, rel) of
    ...
    _ ->
      NRelExp (if rel == LEQ0 then LTH0 else LEQ0)
...
simplifyAndOr is_and fs =
  if LogCt (not is_and) `elem` fs
    then-- False AND p == False
      return [LogCt $ not is_and]
    else-- (i) p AND p == p,        (ii) True AND p == p
    do
      let fs' = S.toList . S.fromList . filter (/= LogCt is_and) $ fs
      if null fs'
        then return [LogCt is_and]
        else do
          -- IF p1 => p2 THEN   p1 AND p2 --> p1
          fs'' <-
            foldM
              ( \l x -> do
                  (addx, l') <- trimImplies is_and x l
    ...
    trimImplies and_case x (p : ps) = do
      ...
      if succc
        then if and_case then trimImplies and_case x ps else return (False, p : ps)
          ...
          suc <- impliesRel p x
          if suc
            then if and_case then return (False, p : ps) else trimImplies and_case x ps
            else do
              (addx, newps) <- trimImplies and_case x ps
              return (addx, p : newps)
...
negateSimplified (SMinus e1 e2) = do
  -- return $ SMinus e2 e1
...
mulVals v1 v2 =
  badAlgSimplifyM $
    "mulVals: operands not of (the same) numeral type! "
      ++ pretty v1
      ++ " "
      ++ pretty v2
...
tryDivProdByOneFact f pev@(NProd (t : tfs) tp, v) = do
  ...
  if not succc
    then do
      (succ', (tfs', v')) <- tryDivProdByOneFact f (NProd tfs tp, v)
      case (succ', tfs') of
