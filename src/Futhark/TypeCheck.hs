...
instance
  ...
  askScope = asks $ M.fromList . mapMaybe varType . M.toList . envVtable
    where
      varType (name, attr) = Just (name, attr)
...
checkFunParams params = foldM_ check mempty params
  where
    param_bound = namesFromList $ map paramName params
    check prev param =
...
checkBasicOp (Update src idxes se) = do
  ...
  when (arrayRank src_t /= length idxes)
    $ bad
    $ SlicingError (arrayRank src_t) (length idxes)
...
checkBasicOp (Repeat shapes innershape v) = do
  ...
  unless (length shapes == arrayRank v_t)
    $ bad
    $ TypeError "Incorrect number of shapes in repeat."
  ...
  binding (scopeOf form) $ do
    ...
    context "Inside the loop body"
      ...
      $ do
        ...
        let rettype_ext =
              existentialiseExtTypes (map paramName mergepat)
                $ staticShapes
                $ map fromDecl rettype
        bodyt <-
          extendedScope (traverse subExpType $ bodyResult loopbody)
            $ scopeOf
            $ bodyStms loopbody
...
checkPatElem ::
  ...
  TypeM lore ()
checkPatElem (PatElem name attr) =
...
matchExtPattern pat ts =
  unless (expExtTypesFromPattern pat == ts)
    $ bad
    $ InvalidPatternError pat ts Nothing
