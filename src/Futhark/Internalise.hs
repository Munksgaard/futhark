...
internaliseValBind fb@(E.ValBind entry fname retdecl (Info (rettype, retext)) tparams params body _ loc) = do
  bindingParams tparams params $ \pcm shapeparams params' -> do
    ...
    let mkConstParam name = Param name $ I.Prim int32
        ...
        constscope =
          M.fromList $ zip constnames $ repeat
            $ FParamInfo
            $ I.Prim
            $ IntType Int32
        shapenames = map I.paramName shapeparams
        normal_params =
          map I.paramName constparams ++ shapenames
...
generateEntryPoint (E.EntryPoint e_paramts e_rettype) (E.ValBind _ ofname _ (Info (rettype, _)) _ params _ _ loc) = do
  ...
  let tparams =
        map (`E.TypeParamDim` noLoc) $ S.toList
          $ mconcat
          $ map E.patternDimNames params_fresh
...
internaliseExp desc (E.Range start maybe_second end (Info ret, Info retext) loc) = do
  ...
  cs <- assertingOne $ do
    ...
    letExp "range_valid_c"
      $ I.BasicOp
      $ I.Assert valid errmsg (loc, mempty)
  step_i32 <- asIntS Int32 step
  pos_step <-
    letSubExp "pos_step"
      $ I.BasicOp
      $ I.BinOp (Mul Int32) step_i32 step_sign_i32
  num_elems <-
    certifying cs $
      letSubExp "num_elems"
  ...
  where
    replace (E.Scalar (E.Record m)) (f : fs) ve' src'
      | Just t <- M.lookup f m = do
        i <-
          fmap sum $ mapM (internalisedTypeSize . snd)
            $ takeWhile ((/= f) . fst)
            $ sortFields m
...
internaliseExp desc (E.Assert e1 e2 (Info check) loc) = do
  ...
  c <-
    assertingOne $ letExp "assert_c"
      $ I.BasicOp
      $ I.Assert e1' (errorMsg [ErrorString check]) (loc, mempty)
...
internaliseDimIndex w (E.DimSlice i j s) = do
  ...
  zero_leq_i_p_m_t_s <-
    letSubExp "zero_leq_i_p_m_t_s"
      $ I.BasicOp
      $ I.CmpOp (I.CmpSle Int32) zero i_p_m_t_s
  i_p_m_t_s_leq_w <-
    letSubExp "i_p_m_t_s_leq_w"
      $ I.BasicOp
      $ I.CmpOp (I.CmpSle Int32) i_p_m_t_s w
  i_p_m_t_s_lth_w <-
    letSubExp "i_p_m_t_s_leq_w"
      $ I.BasicOp
      $ I.CmpOp (I.CmpSlt Int32) i_p_m_t_s w
...
internaliseHist desc rf hist op ne buckets img loc = do
  ...
  buckets'' <-
    certifying c $ letExp (baseString buckets')
      $ I.BasicOp
      $ I.Reshape (reshapeOuter [DimCoercion w_img] 1 b_shape) buckets'
  letTupExp' desc $ I.Op
    $ I.Hist w_img [HistOp w_hist rf' hist' ne_shp op'] (I.Lambda params body' rettype)
    $ buckets'' : img'
...
internaliseStreamRed desc o comm lam0 lam arr = do
  ...
  let form = I.Parallel o comm lam0' accs
      lam' =
        I.Lambda
          { lambdaParams = chunk_param : acc_params ++ lam_val_params,
...
isOverloadedFunction qname args loc = do
  ...
  where
    ...
    handleRest [E.TupLit [e, E.ArrayLit vs _ _] _] "cmp_threshold" = do
      ...
      where
        isCharLit (Literal (SignedValue iv) _) = Just $ chr $ fromIntegral $ intToInt64 iv
        isCharLit _ = Nothing
    ...
    Just (fname, constparams, mk_rettype, bind_ext, _)
      | name `M.notMember` scope -> do
        ...
        case mk_rettype $ zip constargs $ map I.fromDecl const_ts of
          ...
          Just rettype -> do
            ses <-
              fmap (map I.Var) $ letTupExp (baseString name) $
                I.Apply
                  fname
                  (zip constargs const_ds)
                  rettype
...
constFunctionArgs loc = mapM arg
  where
    arg (fname, name) = do
      safety <- askSafety
      se <-
        letSubExp (baseString name ++ "_arg") $
...
funcall desc (QualName _ fname) args loc = do
  ...
  let shapeargs = argShapes shapes value_paramts argts
      diets =
        const_ds ++ replicate (length closure + length shapeargs) I.ObservePrim
          ++ map I.diet value_paramts
      constOrShape = const $ I.Prim int32
      paramts =
        map constOrShape constargs ++ closure_ts
          ++ map constOrShape shapeargs
          ++ map I.fromDecl value_paramts
  args' <-
    ensureArgShapes
      asserting
...
partitionWithSOACS k lam arrs = do
  ...
  add_lam_body <- runBodyBinder
    ...
    $ fmap resultBody
    $ forM (zip add_lam_x_params add_lam_y_params)
    $ \(x, y) ->
      letSubExp "z" $ I.BasicOp $
        I.BinOp
          (I.Add Int32)
          (I.Var $ I.paramName x)
          (I.Var $ I.paramName y)
  ...
  sum_of_partition_sizes <-
    letSubExp "sum_of_partition_sizes"
  ...
  sizes' <-
    letSubExp "partition_sizes" $ I.BasicOp
...
typeExpForError cm (E.TERecord fields _) = do
  ...
  where
    onField (k, te) = (ErrorString (pretty k ++ ": ") :) <$> typeExpForError cm te
...
typeExpForError cm (E.TEApply t arg _) = do
  ...
  arg' <- case arg of
    TypeArgExpType argt -> typeExpForError cm argt
