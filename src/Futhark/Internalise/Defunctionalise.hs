...
defuncFun tparams pats e0 (closure, ret) loc = do
  ...
  let sizes_of_arrays =
      ...
      (fields, env) =
        unzip $ map closureFromDynamicFun
          $ filter (notSize . fst)
          $ M.toList used_env
      env' = M.fromList env
      ...
      let t1 = toStruct $ typeOf e1'
          ...
          fname'' =
            Var
              fname'
              ( Info
                  ( Scalar $ Arrow mempty Unnamed (fromStruct t1)
                      $ Scalar
...
liftValDec fname rettype dims pats body = tell $ Seq.singleton dec
  where
    dims' = map (flip TypeParamDim noLoc) dims
  ...
  return
    ( valbind
        { valBindRetDecl = retdecl,
          valBindRetType =
            Info
              ( combineTypeShapes rettype rettype',
                retext
              ),
