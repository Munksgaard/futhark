...
matchPattern exts loc ses =
  forM (zip exts ses) $ \(et, se) -> do
    ...
    ensureExtShape
      asserting
      (I.ErrorMsg [I.ErrorString "value cannot match pattern"])
      loc
      et'
      "correct_shape"
      se
