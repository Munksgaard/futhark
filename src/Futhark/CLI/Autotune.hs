...
import Data.List (foldl', intersect, isPrefixOf, sort, sortOn)
import Data.Maybe
      ...
      $ \(dataset, do_run) -> do
        ...
        case res of
          ...
          Right _ -> do
            ...
            putStrLn $
              "Calculated timeout for " ++ dataset
                ++ " : "
                ++ show t
                ++ "s"
            return (dataset, do_run t, iosEntryPoint ios)
  where
    run entry_point trun expected timeout path purpose = do
      let opts' = case purpose of
            RunSample -> opts {optRuns = 1}
            RunBenchmark -> opts
      ...
      when (optVerbose opts > 1)
        $ putStrLn
        $ "Running with options: " ++ unwords (runExtraOptions ropts)
                ...
                Just e_par -> do
                  ...
                  case (t_run, f_run) of
                    (Left err, _) -> do
                      when (optVerbose opts > 0)
                        $ putStrLn
                        $ "True comparison run failed:\n" ++ err
                      return prefer_f
                    (_, Left err) -> do
                      when (optVerbose opts > 0)
                        $ putStrLn
                        $ "False comparison run failed:\n" ++ err
                      return prefer_t
