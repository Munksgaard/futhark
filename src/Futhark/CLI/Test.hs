  ...
  case testAction testcase of
    ...
    RunCases ios structures warnings -> do
      ...
      unless (mode == Interpreted)
        $ context ("Compiling with --backend=" <> T.pack backend)
        $ do
          ...
          unless (mode == Compile) $ do
            ...
            context "Running compiled program"
              $ accErrors_
              $ map (runCompiledEntry program progs') ios
      unless (mode == Compile || mode == Compiled)
        $ context "Interpreting"
        $ accErrors_
        $ map (runInterpretedEntry futhark program) ios
...
runTests config paths = do
  ...
  let report
      ...
      clear
        | isTTY = clearFromCursorToScreenEnd
        | otherwise = putStr "\n"
      numTestCases tc =
