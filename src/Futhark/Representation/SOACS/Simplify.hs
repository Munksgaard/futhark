...
simplifySOACS = Simplify.simplifyProg simpleSOACS soacRules blockers
  where
    blockers = Engine.noExtraHoistBlockers {Engine.getArraySizes = getShapeNames}
          ...
          then Skip
            ...
            forM_ copies $ \(from, to) ->
              if UT.isConsumed (patElemName to) used
                then letBind_ (Pattern [] [to]) $ BasicOp $ Copy $ patElemName from
                else letBind_ (Pattern [] [to]) $ BasicOp $ SubExp $ Var $ patElemName from
  where
...
mapOpToOp (_, used) pat aux1 e
  ...
  | Just (map_pe, cs, _, BasicOp (Rotate rots rotate_arr), [p], [arr]) <-
    ...
    Simplify $ certifying (stmAuxCerts aux1 <> cs) $ letBind_ pat
      $ BasicOp
      $ Rotate (intConst Int32 0 : rots) arr
...
removeDeadReduction (_, used) pat (StmAux cs _) (Screma w form arrs)
  | Just ([Reduce comm redlam nes], maplam) <- isRedomapSOAC form,
    ...
    certifying cs $ letBind_ (Pattern [] $ used_red_pes ++ map_pes)
      $ Op
      $ Screma w (redomapSOAC [Reduce comm redlam' used_nes] maplam') arrs
...
simplifyKnownIterationSOAC
  ...
  ( Screma
    ...
    where
      (Reduce _ red_lam red_nes) = singleReduce reds
      ...
      bindMapParam p a = do
        ...
        letBindNames_ [paramName p]
          $ BasicOp
          $ Index a
          $ fullSlice a_t [DimFix $ constant (0 :: Int32)]
      ...
      arr' <-
        if arraySize 0 arr_t == w
          then return arr
            ...
            certifying cs $ letExp (baseString arr ++ "_prefix")
              $ BasicOp
              $ Index arr
