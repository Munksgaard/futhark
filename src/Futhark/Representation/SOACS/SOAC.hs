  ...
  let red_nes = concatMap redNeutral reds
      red_lam =
        let xParams red = take (length (redNeutral red)) (lambdaParams (redLambda red))
         ...
         in Lambda
              { lambdaParams = concatMap xParams reds ++ concatMap yParams reds,
                lambdaReturnType = concatMap (lambdaReturnType . redLambda) reds,
                lambdaBody =
                  mkBody
                    (mconcat (map (bodyStms . lambdaBody . redLambda) reds))
...
scremaType w (ScremaForm (scan_lam, _scan_nes) reds map_lam) =
  ...
  where
    scan_tps = lambdaReturnType scan_lam
...
isScanSOAC form = do
  (scan_lam, scan_nes, map_lam) <- isScanomapSOAC form
...
instance (Attributes lore, CanBeRanged (Op lore)) => CanBeRanged (SOAC lore) where
  ...
  removeOpRanges = runIdentity . mapSOACM remove
    where
      remove = SOACMapper return (return . removeLambdaRanges) return
  ...
  addOpRanges (Hist len ops bucket_fun imgs) =
    Hist
      len
      (map (mapHistOp $ Range.runRangeM . Range.analyseLambda) ops)
      (Range.runRangeM $ Range.analyseLambda bucket_fun)
      imgs
  addOpRanges (Screma w (ScremaForm (scan_lam, scan_nes) reds map_lam) arrs) =
    ...
    where
      onRed red =
        red
          { redLambda =
              Range.runRangeM $ Range.analyseLambda $
                redLambda red
          }
...
instance Annotations lore => ST.IndexOp (SOAC lore) where
  indexOp vtable k soac [i] = do
    ...
    case se of
      ...
      _ -> Nothing
    where
      lambdaAndSubExp (Screma _ (ScremaForm (_, scan_nes) reds map_lam) arrs) =
...
typeCheckSOAC (Scatter w lam ivs as) = do
  ...
  unless (rtsLen == sum as_ns)
    $ TC.bad
    $ TC.TypeError "Scatter: Uneven number of index types, value types, and arrays outputs."
  -- 2.
  forM_ rtsI $ \rtI ->
    unless (Prim int32 == rtI)
      $ TC.bad
      $ TC.TypeError "Scatter: Index return type must be i32."
