...
instance Pretty num => Pretty (LMAD num) where
  ppr (LMAD offset dims) = braces $ semisep
...
instance Pretty num => Pretty (IxFun num) where
  ppr (IxFun lmads oshp cg) = braces $ semisep
...
instance Traversable LMAD where
  traverse f (LMAD offset dims) = LMAD <$> f offset <*> traverse f' dims
   where
    f' (LMADDim s r n p m) =
...
setLMADPermutation perm lmad = lmad
  { lmadDims = zipWith (\dim p -> dim { ldPerm = p }) (lmadDims lmad) perm
  }

setLMADShape :: Shape num -> LMAD num -> LMAD num
setLMADShape shp lmad = lmad
  { lmadDims = zipWith (\dim s -> dim { ldShape = s }) (lmadDims lmad) shp
  }

-- | Substitute a name with a PrimExp in an LMAD.
substituteInLMAD
  :: Ord a => M.Map a (PrimExp a) -> LMAD (PrimExp a) -> LMAD (PrimExp a)
substituteInLMAD tab (LMAD offset dims) =
  let offset' = substituteInPrimExp tab offset
      dims'   = map
        (\(LMADDim s r n p m) -> LMADDim (substituteInPrimExp tab s)
...
substituteInIxFun
  :: (Ord a) => M.Map a (PrimExp a) -> IxFun (PrimExp a) -> IxFun (PrimExp a)
substituteInIxFun tab (IxFun lmads oshp cg) = IxFun
...
hasContiguousPerm (IxFun (lmad :| []) _ _) =
  let perm = lmadPermutation lmad in perm == sort perm
...
index = indexFromLMADs . ixfunLMADs
 where
  indexFromLMADs
    :: (IntegralExp num, Eq num) => NonEmpty (LMAD num) -> Indices num -> num
  ...
  indexFromLMADs (lmad1 :| lmad2 : lmads) inds =
    let i_flat   = indexLMAD lmad1 inds
        new_inds = unflattenIndex
          (permuteFwd (lmadPermutation lmad2) $ lmadShapeBase lmad2)
          i_flat
    in  indexFromLMADs (lmad2 :| lmads) new_inds

  -- | Compute the flat index of an LMAD.
  indexLMAD :: (IntegralExp num, Eq num) => LMAD num -> Indices num -> num
...
repeat (IxFun (lmad@(LMAD off dims) :| lmads) oshp _) shps shp =
  let
    ...
    perm' =
      concatMap (\(p, l) -> map (\i -> (scn !! p) - l + i) [0 .. l - 1])
        $ zip perm lens
    tmp    = length perm'
    perm'' = perm' ++ [tmp .. tmp - 1 + length shp]

    dims'  = concatMap (\(shp_k, srnp) -> map fakeDim shp_k ++ [srnp])
      $ zip shps' dims
    lmad' = setLMADPermutation perm'' $ LMAD off (dims' ++ map fakeDim shp)
  in
    IxFun (lmad' :| lmads) oshp False -- XXX: Can we be less conservative?
...
rotate (IxFun (lmad@(LMAD off dims) :| lmads) oshp cg) offs =
  let dims' = zipWith
        (\(LMADDim s r n p f) o ->
          if s == 0 then LMADDim 0 0 n p Unknown else LMADDim s (r + o) n p f
        )
        dims
...
sliceOneLMAD
  :: (Eq num, IntegralExp num) => IxFun num -> Slice num -> Maybe (IxFun num)
sliceOneLMAD (IxFun (lmad@(LMAD _ ldims) :| lmads) oshp cg) is = do
  ...
  let lmad' = foldl sliceOne (LMAD (lmadOffset lmad) []) $ zip is' ldims
      ...
      perm' = updatePerm perm $ map fst $ filter (isJust . dimFix . snd) $ zip
        [0 .. length is' - 1]
        is'
 ...
 where
    ...
    decrease p =
      let
        d = foldl
          (\n i -> if i == p
            then -1
            else if i > p then n else if n /= -1 then n + 1 else n
          )
          0
          inds
      in  [ p - d | d /= -1 ]
  ...
  harmlessRotation'
    :: (Eq num, IntegralExp num) => LMADDim num -> DimIndex num -> Bool
  ...
  harmlessRotation' (LMADDim _ _ n _ _) dslc
    | dslc == DimSlice (n - 1) n (-1) || dslc == unitSlice 0 n = True
  harmlessRotation' _ _ = False

  harmlessRotation :: (Eq num, IntegralExp num) => LMAD num -> Slice num -> Bool
  harmlessRotation (LMAD _ dims) iss = and $ zipWith harmlessRotation' dims iss
  ...
  slicePreservesContiguous
    :: (Eq num, IntegralExp num) => LMAD num -> Slice num -> Bool
  slicePreservesContiguous (LMAD _ dims) slc =
    ...
    let
      (dims', slc') =
        unzip $ filter ((/= 0) . ldStride . fst) $ zip dims $ map normIndex slc
      -- Check that:
  ...
  normIndex :: (Eq num, IntegralExp num) => DimIndex num -> DimIndex num
...
slice ixfun@(IxFun (lmad@(LMAD _ _) :| lmads) oshp cg) dim_slices
  ...
  | otherwise = case sliceOneLMAD (iota (lmadShape lmad)) dim_slices of
...
reshapeCoercion (IxFun (lmad@(LMAD off dims) :| lmads) _ cg) newshape = do
  ...
  let dims'' = map snd $ sortBy (compare `on` fst) $ zipWith
        (\ld n -> (ldPerm ld, ld { ldShape = n }))
        dims'
        (newDims newshape)
      lmad' = LMAD off dims''
  ...
  let rsh_len    = length reshapes
        ...
        foldl
            (\(sup, rpt) (i, shpdim, ip) ->
              case (i < hd_len, i >= hd_len + rsh_len, shpdim) of
                (True, _, DimCoercion n) -> case dims_perm !! i of
                  (LMADDim 0 _ _ _ _) -> (sup, (ip, n) : rpt)
                  (LMADDim _ r _ _ _) -> ((ip, (r, n)) : sup, rpt)
                (_, True, DimCoercion n) -> case dims_perm !! (i - diff) of
                  (LMADDim 0 _ _ _ _) -> (sup, (ip, n) : rpt)
                  (LMADDim _ r _ _ _) -> ((ip, (r, n)) : sup, rpt)
                (False, False, _) -> ((ip, (0, newDim shpdim)) : sup, rpt)
      ...
      dims' =
        map snd
          ...
          $  zip sup_inds dims_sup
          ++ zip rpt_inds repeats'
      lmad' = LMAD off' dims'
...
reshape
  :: (Eq num, IntegralExp num) => IxFun num -> ShapeChange num -> IxFun num
...
rebaseNice
  :: (Eq num, IntegralExp num) => IxFun num -> IxFun num -> Maybe (IxFun num)
rebaseNice new_base@(IxFun (lmad_base :| lmads_base) _ cg_base) ixfun@(IxFun lmads shp cg)
  = do
    ...
    let perm_base' = if hasContiguousPerm ixfun
        ...
        (dims_base', offs_contrib) = unzip $ zipWith
 ...
 where
  ...
  -- @res@, such that @repeat r res@ is identical to the input lmad.
    let
      ...
      resacc =
        foldl
            (\acc (LMADDim s _ n _ _) -> case acc of
              rpt : acc0 ->
                if s == 0 then (n : rpt) : acc0 else [] : (rpt : acc0)
              _ -> error "shaveoffRepeats: empty accumulator"
            )
            [[]]
          $ reverse
          $ permuteFwd perm dims
      last_shape = last resacc
      shapes     = take (length resacc - 1) resacc
      -- update permutation and lmad:
      howManyRepLT k = foldl
        (\i (LMADDim s _ _ p _) -> if s == 0 && p < k then i + 1 else i)
        0
        dims
      dims' =
        foldl
            (\acc (LMADDim s r n p info) -> if s == 0
              then acc
              else let p' = p - howManyRepLT p in LMADDim s r n p' info : acc
            )
            []
          $ reverse dims
      lmad' = LMAD (lmadOffset lmad) dims'
    in
      ((shapes, last_shape), lmad')
...
rebase new_base@(IxFun lmads_base shp_base cg_base) ixfun@(IxFun lmads shp cg)
  | Just ixfun' <- rebaseNice new_base ixfun
  = ixfun'
  |
  -- In the general case just concatenate LMADs since this refers to index
  -- function composition, which is always safe.
    otherwise
  = let (lmads_base', shp_base') = if base ixfun == shape new_base
          then (lmads_base, shp_base)
          else
            let IxFun lmads' shp_base'' _ =
                  reshape new_base $ map DimCoercion shp
            in  (lmads', shp_base'')
...
offsetIndex ixfun i          = case shape ixfun of
...
strideIndex ixfun s = case shape ixfun of
...
rearrangeWithOffset
  :: (Eq num, IntegralExp num) => IxFun num -> num -> Maybe (num, [(Int, num)])
...
flatOneDim :: (Eq num, IntegralExp num) => (num, num, num) -> num -> num
flatOneDim (s, r, n) i | s == 0    = 0
                       | r == 0    = i * s
                       | otherwise = ((i + r) `mod` n) * s

-- | Generalised iota with user-specified offset and strides.
makeRotIota
  :: IntegralExp num => Monotonicity -> num -> [(num, num)] -> LMAD num
makeRotIota mon off support
  | mon == Inc || mon == Dec
  = let rk       = length support
        (rs, ns) = unzip support
        ss0      = reverse $ take rk $ scanl (*) 1 $ reverse ns
        ss       = if mon == Inc then ss0 else map (* (-1)) ss0
        ps       = map fromIntegral [0 .. rk - 1]
        fi       = replicate rk mon
    in  LMAD off $ zipWith5 LMADDim ss rs ns ps fi
  | otherwise
  = error "makeRotIota: requires Inc or Dec"

-- | Check monotonicity of an index function.
ixfunMonotonicityRots
  :: (Eq num, IntegralExp num) => Bool -> IxFun num -> Monotonicity
ixfunMonotonicityRots ignore_rots (IxFun (lmad :| lmads) _ _) =
  let mon0 = lmadMonotonicityRots lmad
  in  if all ((== mon0) . lmadMonotonicityRots) lmads then mon0 else Unknown
 where
  lmadMonotonicityRots :: (Eq num, IntegralExp num) => LMAD num -> Monotonicity
  lmadMonotonicityRots (LMAD _ dims) | all (isMonDim Inc) dims = Inc
                                     | all (isMonDim Dec) dims = Dec
                                     | otherwise               = Unknown

  isMonDim :: (Eq num, IntegralExp num) => Monotonicity -> LMADDim num -> Bool
