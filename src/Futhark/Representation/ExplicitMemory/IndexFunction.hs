...
repeat (IxFun (lmad@(LMAD off dims) :| lmads) oshp _) shps shp =
  ...
  where
    fakeDim x = LMADDim 0 0 x 0 Unknown
...
sliceOneLMAD (IxFun (lmad@(LMAD _ ldims) :| lmads) oshp cg) is = do
  ...
  let lmad' = foldl sliceOne (LMAD (lmadOffset lmad) []) $ zip is' ldims
      ...
      perm' =
        updatePerm perm $ map fst $ filter (isJust . dimFix . snd) $
...
rebaseNice
  ...
  ixfun@(IxFun lmads shp cg) = do
    ...
    let perm_base' =
        ...
        IxFun lmads_base' _ _ =
          if all null outer_shapes && null inner_shape
            then new_base'
    ...
    where
      ...
      shaveoffRepeats lmad =
        ...
        let perm = lmadPermutation lmad
            ...
            dims' =
              foldl
                ( \acc (LMADDim s r n p info) ->
                    if s == 0
                      then acc
                      else
                        let p' = p - howManyRepLT p
