...
addKernelBodyRanges ::
  ...
  KernelBody lore ->
  Range.RangeM (KernelBody (Ranges lore))
...
data SegOpMapper flore tlore m
  = SegOpMapper
...
mapSegOpM tv (SegRed lvl space reds ts lam) =
  ...
  where
    onSegOp (SegRedOp comm red_op nes shape) =
      SegRedOp comm
...
instance Attributes lore => Rename (SegOp lore) where
  rename = mapSegOpM renamer
    where
      renamer = SegOpMapper rename rename rename rename

instance
  (Attributes lore, FreeIn (LParamAttr lore)) =>
  FreeIn (SegOp lore)
  where
...
instance OpMetrics (Op lore) => OpMetrics (SegOp lore) where
  ...
  opMetrics (SegRed _ _ reds _ body) =
    inside "SegRed" $ do
      mapM_ (lambdaMetrics . segRedLambda) reds
...
instance PrettyLore lore => PP.Pretty (SegOp lore) where
  ...
  ppr (SegRed lvl space reds ts body) =
    text "segred_" <> ppr lvl
        ...
        <+> PP.nestedBlock "{" "}" (ppr body)
    where
      ppOp (SegRedOp comm lam nes shape) =
