...
simplifyKernelOp _ (SegOp (SegMap lvl space ts kbody)) = do
  ...
  return
    ( SegOp $ SegMap lvl' space' ts' kbody',
      body_hoisted
    )
simplifyKernelOp _ (SegOp (SegRed lvl space reds ts kbody)) = do
  ...
  (kbody', body_hoisted) <- simplifyKernelBody space kbody
  return
    ( SegOp $ SegRed lvl' space' reds' ts' kbody',
...
simplifyKernelOp _ (SegOp (SegScan lvl space scan_op nes ts kbody)) = do
  ...
  return
    ( SegOp $ SegScan lvl' space' scan_op' nes' ts' kbody',
      hoisted
    )
simplifyKernelOp _ (SegOp (SegHist lvl space ops ts kbody)) = do
  ...
  (kbody', body_hoisted) <- simplifyKernelBody space kbody
  return
    ( SegOp $ SegHist lvl' space' ops' ts' kbody',
...
simplifyRedOrScan ::
  (Engine.SimplifiableLore lore, BodyAttr lore ~ ()) =>
  SegSpace ->
  Lambda lore ->
  [SubExp] ->
  [Type] ->
...
simplifyKernelBody space (KernelBody _ stms res) = do
  ...
  return
    ( mkWiseKernelBody () body_stms body_res,
      hoisted
    )
  where
    scope_vtable = ST.fromScope $ scopeOfSegSpace space
...
kernelRules =
  standardRules
    <> ruleBook
      [ RuleOp removeInvariantKernelResults,
...
distributeKernelResults
  ...
  (Pattern [] kpes)
  attr
  (SegOp (SegMap lvl space kts (KernelBody _ kstms kres))) = Simplify $ do
        ...
        | Let (Pattern [] [pe]) _ _ <- stm,
          ...
          let outer_slice =
                map
                  ...
                  $ segSpaceDims space
              index kpe' =
                letBind_ (Pattern [] [kpe']) $ BasicOp $ Index arr $
...
mergeSegRedOps _ (Pattern [] pes) _ (SegOp (SegRed lvl space ops ts kbody))
  | length ops > 1,
    op_groupings <-
      groupBy sameShape $ zip ops $ chunks (map (length . segRedNeutral) ops) $
  ...
  where
    (red_pes, map_pes) = splitAt (segRedResults ops) pes
    ...
    combine (op1, op1_aux) (op2, op2_aux) =
      let lam1 = segRedLambda op1
       ...
       in ( SegRedOp
              { segRedComm = segRedComm op1 <> segRedComm op2,
                segRedLambda = lam,
