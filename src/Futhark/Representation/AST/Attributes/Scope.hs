...
instance
  (Applicative m, Monad m, Annotations lore) =>
...
instance
  (Applicative m, Monad m, Monoid w, Annotations lore) =>
...
scopeOfLParams = M.fromList . map f
  where
    f param = (paramName param, LParamInfo $ paramAttr param)
