...
import Futhark.Util.IntegralExp (IntegralExp, quot, quotRoundingUp)
...
opCompiler (Pattern _ [pe]) (Inner (SizeOp (CalcNumGroups w64 max_num_groups_key group_size))) = do
  ...
  let num_groups_maybe_zero =
        BinOpExp
          (SMin Int64)
          ( toExp' int64 w64
              `quotRoundingUp` i64 (toExp' int32 group_size)
          )
  ...
  where
    i64 = ConvOpExp (SExt Int32 Int64)
...
callKernelCopy
  ...
  srcloc@(MemLocation srcmem srcshape srcIxFun)
    | Just
        ...
        isMapTransposeKernel bt destloc srcloc = do
