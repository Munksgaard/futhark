...
onKernel target kernel = do
  ...
  let (kernel_body, cstate) =
        GenericC.runCompilerM
          (inKernelOperations (kernelBody kernel))
          blankNameSource
          (newKernelState failures)
          $ GenericC.blockScope
          $ GenericC.compileCode
          $ kernelBody kernel
...
inKernelOperations body =
  ...
  where
    ...
    errorInKernel msg@(ErrorMsg parts) backtrace = do
      ...
      let what_next
            | has_communication =
              [C.citems|local_failure = true;
                                                goto $id:label;|]
            | otherwise = [C.citems|return;|]
      GenericC.stm
        [C.cstm|{ if (atomic_cmpxchg(global_failure, -1, $int:n) == -1)
                                 { $stms:argstms; }
                                 $items:what_next
                               }|]
