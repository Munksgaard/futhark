...
computeHistoUsage space op = do
  ...
  subhisto_infos <- forM (zip (histDest op) (histNeutral op)) $ \(dest, ne) -> do
    ...
    let subhistos_shape =
          Shape (map snd segment_dims ++ [Var num_subhistos])
            <> stripDims num_segments (arrayShape dest_t)
        subhistos_membind =
          ArrayIn subhistos_mem $ IxFun.iota
    ...
    return $ SubhistosInfo subhistos $ do
      let unitHistoCase =
          ...
          multiHistoCase = do
            let num_elems =
                  foldl' (*) (Imp.var num_subhistos int32)
                    $ map (toExp' int32)
                    $ arrayDims dest_t
            ...
            let slice =
                  fullSliceNum (map (toExp' int32) $ arrayDims subhistos_t) $
      ...
      <-- case passage of
        MayBeMultiPass ->
          (hist_M_min * hist_H * hist_el_size)
            `quotRoundingUp` t64 (hist_F_L2 * r64 (Imp.vi32 hist_L2) * hist_RACE_exp)
        MustBeSinglePass ->
          1
  ...
  sKernelThread "seghist_global" num_groups group_size (segFlat space) $ \constants -> do
    ...
    subhisto_inds <- forM slugs $ \slug ->
      dPrimVE "subhisto_ind" $
        kernelGlobalThreadId constants
          `quot` (kernelNumThreads constants `quotRoundingUp` Imp.vi32 (slugNumSubhistos slug))
    ...
    kernelLoop gtid num_threads total_w_64 $ \offset -> do
      ...
      zipWithM_ setIndex space_is
        ...
        $ unflattenIndex space_sizes_64 offset
        ...
        case do_op of
          ...
          AtomicLocking f -> return $ \(constants, hist_H_chk) -> do
            ...
            sComment "All locks start out unlocked"
              $ groupCoverSpace constants dims
              $ \is ->
    ...
    sKernelThread "seghist_local" num_groups group_size (segFlat space) $ \constants ->
      virtualiseGroups constants SegVirt (unCount groups_per_segment * num_segments) $ \group_id_var -> do
              ...
              onSlugs $ \slug dests hist_H_chk histo_dims histo_size -> do
                ...
                init_per_thread <-
                  dPrimVE "init_per_thread" $
                    group_hists_size
                      `quotRoundingUp` kernelGroupSize constants
        ...
        sComment "initialize histograms in local memory"
          $ onAllHistograms
          $ \dest_local dest_global op ne local_subhisto_i global_subhisto_i local_bucket_is global_bucket_is ->
        ...
        sComment "Compact the multiple local memory subhistograms to result in global memory"
          $ onSlugs
          $ \slug dests hist_H_chk histo_dims histo_size -> do
            bins_per_thread <-
              dPrimVE "init_per_thread" $
            ...
            sFor "local_i" bins_per_thread $ \i -> do
              ...
              sWhen (j .<. trunc_histo_size) $ do
                ...
                sComment "Accumulate based on values in other subhistograms."
                  $ sFor "subhisto_id" (num_subhistos_per_group - 1)
                  $ \subhisto_id -> do
...
localMemoryCase map_pes hist_T space hist_H hist_el_size hist_N hist_RF slugs kbody = do
  ...
  num_groups <-
    fmap (Imp.Count . Var) $ dPrimV "num_groups" $
  ...
  hist_M0 <-
    if any (lockOp . slugAtomicUpdate) slugs
      then lockCase
        ...
        if any (casOp . slugAtomicUpdate) slugs
          then casCase
          else hwdCase
      ...
      then do
        hist_T_hist_min <-
          dPrimVE "hist_T_hist_min"
            ...
            $ Imp.BinOpExp
              (SMin Int64)
              (i32_to_i64 hist_Nin * i32_to_i64 hist_Nout)
              (i32_to_i64 hist_T)
              `quotRoundingUp` i32_to_i64 hist_Nout
...
compileSegHist (Pattern _ pes) num_groups group_size space ops kbody = do
  ...
  sUnless (seg_h .==. 0) $ do
    ...
    let lockSize slug = case slugAtomicUpdate slug of
          ...
          _ -> Nothing
    hist_el_size <-
      dPrimVE "hist_el_size" $ foldl' (+) (h `quotRoundingUp` hist_H) $
    ...
    emit $ Imp.DebugPrint "Number of segments"
      $ Just
      $ product
      $ map (toExp' int32 . snd) segment_dims
