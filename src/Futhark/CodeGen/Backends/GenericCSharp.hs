...
newtype CompilerEnv op s
  = CompilerEnv
      { envOperations :: Operations op s
      }

data CompilerAcc op s
  = CompilerAcc
      { accItems :: [CSStmt],
        accFreedMem :: [VName]
      }
...
compileOutput = nameFun &&& typeFun
  where
    nameFun = Var . compileName . Imp.paramName
...
compileProg module_name constructor imports defines ops userstate boilerplate pre_timing _ options (Imp.Functions funs) = do
  ...
  where
    ...
    main_entry :: [CSStmt]
    main_entry =
      [ Assign (Var "internalInstance") (simpleInitClass "FutharkInternal" [Var "args"]),
    ...
    at_inits =
      [ Reassign (Var "DoWarmupRun") (Bool False),
    ...
    selectEntryPoint entry_point_names entry_points =
      [ Assign (Var "EntryPoints")
          $ Collection "Dictionary<string, Action>"
          $ zipWith Pair (map String entry_point_names) entry_points,
        If
          (simpleCall "!EntryPoints.ContainsKey" [Var "EntryPoint"])
          [ Exp $
              simpleCall
...
unpackDim arr_name (Imp.Var var) i = do
  ...
  if isAssigned
    then stm $ Reassign dest $ Cast (Primitive $ CSInt Int32T) src
    else do
...
entryPointOutput (Imp.TransparentValue (Imp.ScalarValue bt ept name)) =
  ...
  where
    cast = compileTypecastExt bt ept
...
printStm (Imp.ArrayValue mem space bt ept (outer : shape)) ind e = do
  ...
  let dims = map compileDim $ outer : shape
      size =
        callMethod
          (CreateArray (Primitive $ CSInt Int32T) $ Right dims)
          "Aggregate"
          [ Integer 1,
            Lambda
              (Tuple [Var "acc", Var "val"])
  ...
  where
    ppArrayType t [] = prettyPrimType ept t
...
printValue = fmap concat . mapM (uncurry printValue')
  where
    ...
    printValue' (Imp.TransparentValue r@Imp.ArrayValue {}) e = do
      ...
      let e' = Var $ pretty e
      return
        [ Assign (Var $ compileName tuple) (Field e' "Item1"),
          Assign i' (Integer 0),
          p,
...
prepareEntry (fname, Imp.Function _ outputs inputs _ results args) = do
  ...
  (argexps_mem_copies, prepare_run) <- collect' $ forM inputs $ \case
    ...
    _ -> return Nothing
  prepareIn <-
    collect $ mapM_ entryPointInput $ zip3 [0 ..] args $
...
copyMemoryDefaultSpace destmem destidx srcmem srcidx nbytes =
  stm $ Exp $
    simpleCall
      "Buffer.BlockCopy"
      [ Var (compileName srcmem),
        srcidx,
        Var (compileName destmem),
        destidx,
        nbytes
      ]
...
compileEntryFun pre_timing entry@(_, Imp.Function _ outputs _ _ results args) = do
  ...
  where
    getType :: Imp.ExternalValue -> CSType
...
callEntryFun pre_timing entry@(fname, Imp.Function _ outputs _ _ _ decl_args) =
  if any isOpaque decl_args
    then return (Def fname' VoidT [] [exitException], nameToString fname, Var fname')
    else do
      ...
      let maybe_free =
            [ If
                (BinOp "<" (Var "i") (BinOp "-" (Var "NumRuns") (Integer 1)))
                prepare_out
                []
            ]
          do_warmup_run =
  ...
  where
    fname' = "entry_" ++ nameToString fname
...
addTiming statements = do
  ...
  return
    ( [ Assign (Var "StopWatch") $ simpleInitClass "Stopwatch" [],
        syncFun,
  ...
  where
    print_runtime = Exp $ simpleCall "RuntimeFileWriter.WriteLine" [callMethod (Var "timeElapsed") "ToString" []]
    not_null var = BinOp "!=" var Null
    asMicroseconds watch =
      BinOp
        "/"
        (Field watch "ElapsedTicks")
...
compileCode (Imp.Call dests fname args) = do
  ...
  where
    compileArg (Imp.MemArg m) = return $ Var $ compileName m
