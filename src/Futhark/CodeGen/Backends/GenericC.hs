...
defError (ErrorMsg parts) stacktrace = do
  ...
  stm
    [C.cstm|{ ctx->error = msgprintf($string:formatstr, $args:formatargs, $string:stacktrace);
                $items:free_all_mem
                return 1;
              }|]
...
defineMemorySpace space = do
  ...
  alloc <- collect $
    case space of
      Space sid ->
        join $
          asks envAllocate <*> pure [C.cexp|block->mem|]
            <*> pure [C.cexp|size|]
            <*> pure [C.cexp|desc|]
            <*> pure sid
      _ ->
...
allocMem name size space on_failure = do
  ...
  where
    alloc dest = case space of
      Space sid ->
        join $
          asks envAllocate <*> rawMem name
...
$edecls:entry_point_decls
  ...
  where
    compileProg' = do
      ...
      libDecl
        [C.cedecl|void futhark_debugging_report($ty:ctx_ty *ctx) {
...
compileFun (fname, Function _ outputs inputs body _ _) = do
  ...
  ctx_ty <- contextType
  return
    ( [C.cedecl|static int $id:(funName fname)($ty:ctx_ty *ctx,
