...
import Futhark.CodeGen.ImpCode.OpenCL hiding (If, Index, SubExp (..))
...
failureCase i (FailureMsg (ErrorMsg parts) backtrace) =
  If
    (BinOp "==" (Var "failure_idx") (Integer i))
...
generateBoilerplate opencl_code opencl_prelude kernels types sizes failures = do
    ...
    CS.privateFunDef
      ...
      [ Assign tmp_cfg $ CS.simpleInitClass cfg [],
        ...
        Exp $
          CS.simpleCall
            "OpenCLConfigInit"
            [ Out $ Field tmp_cfg "OpenCL",
              (Integer . toInteger) $ M.size sizes,
              Var "SizeNames",
              Var "SizeVars",
              Field tmp_cfg "Sizes",
              Var "SizeClasses"
  ...
  CS.stm $
    CS.privateFunDef
      cfg_set_debugging
      VoidT
      [(RefT $ CustomT cfg, "_cfg"), (Primitive BoolT, "flag")]
      [Reassign (Var "_cfg.OpenCL.Debugging") (Var "flag")]
  CS.stm $
    CS.privateFunDef
      cfg_set_device
      VoidT
      [(RefT $ CustomT cfg, "_cfg"), (stringT, "s")]
  ...
  CS.stm $
    CS.privateFunDef
      ...
      [ AST.For
          ...
          ((Integer . toInteger) $ M.size sizes)
          [ If
              (BinOp "==" (Var "SizeName") (Index (Var "SizeNames") (IdxExp (Var "i"))))
  ...
  let set_required_types =
        [ Reassign (Var "RequiredTypes") (AST.Bool True)
      ...
      set_sizes =
        zipWith
          ( \i k ->
              Reassign
                (Field (Var "Ctx.Sizes") (zEncodeString $ pretty k))
  ...
  CS.stm $
    CS.privateFunDef
      ...
      [ AssignTyped intT (Var "failure_idx") (Just $ CS.simpleInitClass (pretty intT) []),
        Unsafe
          [ CS.assignScalarPointer (Var "failure_idx") (Var "ptr"),
            Reassign (Var "Ctx.GlobalFailureIsAnOption") (Integer 0),
            Exp $
              CS.simpleCall
                "OPENCL_SUCCEED"
        ...
        If
          (BinOp "!=" (Var "failure_idx") (Integer (-1)))
          ( [ AssignTyped intArrayT (Var "args") $ Just $ CreateArray intT
              ...
              Unsafe
                [ Fixed
                    (Var "ptr")
                    (Addr (Index (Var "args") $ IdxExp $ Integer 0))
                    [ Exp $
                        CS.simpleCall
                          "CL10.EnqueueReadBuffer"
                          [ Var "Ctx.OpenCL.Queue",
                            Var "Ctx.GlobalFailureArgs",
                            AST.Bool True,
                            CS.toIntPtr $ Integer 0,
  ...
  where
    ...
    ctx_inits =
      [ Reassign (Var $ ctx "TotalRuns") (Integer 0),
        ...
        ++ concatMap
          ( \name ->
              [ Reassign (Var $ (ctx . kernelRuntime) name) (Integer 0),
    ...
    openCL_load =
      [ CS.privateFunDef
          ...
          [ (RefT $ CustomT futhark_context, "Ctx"),
            (RefT $ CustomT "OpenCLDeviceOption", "Option")
          ]
          $ map sizeHeuristicsCode sizeHeuristicsTable
      ]
    openCL_boilerplate =
      AssignTyped
        stringArrayT
...
openClReport names =
  ...
  where
    longest_name = foldl max 0 $ map length names
...
sizeHeuristicsCode (SizeHeuristic platform_name device_type which what) =
  let which'' = BinOp "==" which' (Integer 0)
   ...
   in If
        ...
        []
  where
    clDeviceType DeviceGPU = "ComputeDeviceTypes.Gpu"
        ...
        Unsafe
          [ Fixed
              ...
              [ Exp $
                  CS.simpleCall
                    "CL10.GetDeviceInfo"
                    [ Var "Ctx.OpenCL.Device",
                      Var "ComputeDeviceInfo.MaxComputeUnits",
