...
import Futhark.Representation.ExplicitMemory (ExplicitMemory, Prog, int32)
...
compileProg module_name prog = do
  ...
  CS.compileProg
    ...
    prog'
  where
    operations :: CS.Operations Imp.OpenCL ()
    operations =
...
launchKernel safety kernel_name kernel_dims workgroup_dims args = do
  ...
  args_stms <-
    zipWithM
      (processKernelArg kernel_name')
      [toInteger (Imp.numFailureParams safety) ..]
      args
  ...
  let debugStartStmts =
        map Exp $
          [CS.consoleErrorWrite "Launching {0} with global work size [" [String kernel_name]]
  ...
  CS.stm $
    If
      (BinOp "!=" total_elements (Integer 0))
  ...
  when (safety >= Imp.SafetyFull)
    $ CS.stm
    $ Reassign (Var "Ctx.GlobalFailureIsAnOption") (Integer 1)
  finishIfSynchronous
  where
    processMemArg kernel argnum mem = do
      err <- newVName' "setargErr"
      dest <- newVName "kArgDest"
      let err_var = Var err
      return
        [ Fixed
            (Var $ CS.compileName dest)
            (Addr mem)
...
copyOpenCLMemory destmem destidx Imp.DefaultSpace srcmem srcidx (Imp.Space "device") nbytes _ = do
  ...
  CS.stm $
    Fixed
      (Var ptr)
      (Addr $ Index destmem' $ IdxExp $ Integer 0)
      [ ifNotZeroSize nbytes
          $ Exp
          $ CS.simpleCall
            "CL10.EnqueueReadBuffer"
            [ Var "Ctx.Opencl.Queue",
              memblockFromMem srcmem,
              Bool True,
              CS.toIntPtr srcidx,
              nbytes,
              CS.toIntPtr $ Var ptr,
...
copyOpenCLMemory destmem destidx (Imp.Space "device") srcmem srcidx Imp.DefaultSpace nbytes _ = do
  ...
  CS.stm $
    Fixed
      (Var ptr)
      (Addr $ Index (Var srcmem') $ IdxExp $ Integer 0)
      [ ifNotZeroSize nbytes
          $ Exp
          $ CS.simpleCall
            "CL10.EnqueueWriteBuffer"
            [ Var "Ctx.OpenCL.Queue",
              memblockFromMem destmem,
              Bool True,
              CS.toIntPtr destidx,
              CS.toIntPtr nbytes,
              CS.toIntPtr $ Var ptr,
...
staticOpenCLArray name "device" t vs = do
  ...
  CS.staticMemAlloc $
    Unsafe
      [ Fixed
          (Var ptr)
          (Addr $ Index (Var tmp_arr) $ IdxExp $ Integer 0)
          [ ifNotZeroSize size
              $ Exp
              $ CS.simpleCall
                "CL10.EnqueueWriteBuffer"
                [ Var "Ctx.OpenCL.Queue",
                  memblockFromMem name,
                  Bool True,
...
unpackArrayInput mem "device" t _ dims e = do
  ...
  CS.stm $
    Unsafe
      [ Fixed
          (Var ptr)
          (Addr $ Index (Field e "Item1") $ IdxExp $ Integer 0)
          [ ifNotZeroSize nbytes
              $ Exp
              $ CS.simpleCall
                "CL10.EnqueueWriteBuffer"
