...
data CSIdx
  = IdxRange CSExp CSExp
  | IdxExp CSExp
  deriving (Eq, Show)

data CSArg
  = ArgKeyword String CSArg -- please don't assign multiple keywords with the same argument
...
instance Pretty CSStmt where
  ...
  ppr (For i what body) =
    ...
    where
      initialize = text "int" <+> text i <+> text "= 0" <+> semi
  ...
  ppr (ForEach i what body) =
    ...
    where
      initialize = text "var" <+> text i <+> text "in " <+> ppr what
